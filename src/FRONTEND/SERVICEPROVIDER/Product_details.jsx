...
const ProductDetails = () => {
  ...
  useEffect(() => {
    axios.get("/categories").then((r) => setCategories(r.data)).catch(console.error);
  }, []);
  ...
  const handleCategoryChange = async (e) => {
    ...
    try {
      const res = await axios.get(`/categories/${id}/subcategories`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post("/add-product", fd, { headers: { "Content-Type": "multipart/form-data" } });
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await axios.get(`/api/product/${searchId}`);
      setProduct(res.data);
      setEditMode(false);
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      await axios.put(`/api/update-product/${product.product_id}`, {
